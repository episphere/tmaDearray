...
svs.getDownloadURL = async (id="774059009195") => {
...
window.onload = async () => {
  ...
  document.getElementById("circleRadiusSlider").onclick = stopPropagation
  document.getElementById("onlyContours").onclick = stopPropagation
}
...
svs.dearrayTMA = () => {
  const circleRadiusThreshold = parseFloat(document.getElementById("circleRadiusSlider").value) || 15
  const onlyContours = document.getElementById("onlyContours").checked
  if (document.body.lastElementChild.id !== "canvasOutput") {
    const canvasOutput = document.createElement("canvas")
    canvasOutput.id = "canvasOutput"
    canvasOutput.width = viewer1.canvas.clientWidth
    canvasOutput.width = viewer1.canvas.clientHeight
    document.body.appendChild(canvasOutput)
    canvasOutput.scrollIntoView()
  }

  let src = cv.imread(viewer1.canvas.querySelector("canvas"))
  let dst = cv.Mat.zeros(src.rows, src.cols, cv.CV_8UC3);
  let outputImgData = undefined
  if (onlyContours)
    outputImgData = cv.Mat.zeros(src.rows, src.cols, cv.CV_8UC3);
  cv.cvtColor(src, dst, cv.COLOR_RGBA2GRAY, 0)
  cv.medianBlur(dst, dst, 9)
  cv.threshold(dst, dst, 230, 200, cv.THRESH_BINARY)
  let contours = new cv.MatVector()
  let hierarchy = new cv.Mat()
  cv.findContours(dst, contours, hierarchy, cv.RETR_CCOMP, cv.CHAIN_APPROX_TC89_KCOS)
  for (let i = 0; i < contours.size(); ++i) {
      let color = new cv.Scalar(255,0,0,255)
      let circle = cv.minEnclosingCircle(contours.get(i))
      if (circle.radius > circleRadiusThreshold) {
        if (outputImgData) {
          cv.circle(outputImgData, circle.center, circle.radius, color);
        } else{
  ...
  if (outputImgData) {
    cv.imshow('canvasOutput', outputImgData)
  } else{
